document.addEventListener("DOMContentLoaded", () => {
  const animateTextElements = (selector, splitBy) => {
    ...
    textContainers.forEach((textContainers) => {
      let elements = [];
      let elementType = "";

      if (splitBy === "words") {
        elements = textContainers.textContent.trim().split(/\s+/);
        elementType = "word";
      } else if (splitBy === "letters") {
        const words = textContainers.textContent.trim().split(/\s+/);
        elements = [];
      ...
      textContainers.textContent = "";

      const animatedElements = [];
      elements.forEach((element, index) => {
        if (splitBy === "letters" && element === " ") {
          textContainers.appendChild(document.createTextNode(" "));
          return;
        }
        ...
        textContainers.appendChild(elementSpan);
